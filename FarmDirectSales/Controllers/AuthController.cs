...
namespace FarmDirectSales.Controllers
{
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            try
            {
                ...
                if (request.Role == "farmer")
                {
                    ...
                    {
                        // 优先使用嵌套的 FarmerProfile 数据，如果没有则使用平坦结构
                        var farmName = request.FarmerProfile?.FarmName ?? request.FarmName ?? string.Empty;
                        var location = request.FarmerProfile?.Location ?? request.Location ?? string.Empty;
                        var description = request.FarmerProfile?.Description ?? request.Description;
                        var productCategory = request.FarmerProfile?.ProductCategory ?? request.ProductCategory;
                        var licenseNumber = request.FarmerProfile?.LicenseNumber ?? request.LicenseNumber;
    ...
    public class RegisterRequest
    {
        ...
        /// 农户资料（嵌套对象）
        /// </summary>
        public RegisterFarmerProfile? FarmerProfile { get; set; }
    ...
    /// 注册时的农户资料模型
    /// </summary>
    public class RegisterFarmerProfile
    {
        /// <summary>
        /// 农场名称
        /// </summary>
