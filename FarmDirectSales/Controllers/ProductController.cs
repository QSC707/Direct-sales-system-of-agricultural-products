    ...
    public class ProductController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            try
            {
                ...
                {
                    // 管理员可以查看所有产品（包括下架的）
                    Console.WriteLine("管理员筛选: 返回所有产品");
                }
                else
                {
                    // 普通用户只返回上架(IsActive)的产品
                    query = query.Where(p => p.IsActive);
                    Console.WriteLine("普通用户筛选: 只返回上架产品");
                }
                ...
                // 关键词搜索 - 搜索产品名称、描述和农户用户名
                if (!string.IsNullOrEmpty(keyword))
                {
                    query = query.Where(p => p.ProductName.Contains(keyword) || 
                                           p.Description.Contains(keyword) ||
                                           p.Farmer.Username.Contains(keyword));
                    Console.WriteLine($"应用关键词搜索: keyword = '{keyword}' (搜索产品名称、描述、农户用户名)");
                }
                else
                {
                ...
                return Ok(new
                {
                    ...
                    data = products.Select(p => new
                    {
                        ...
                        p.Price,
                        p.Stock,
                        ImageUrl = ValidateImageUrl(p.ImageUrl), // 验证图片文件是否存在
                        p.Category,
                        p.Specification,
                        p.IsOrganic,
                        p.HarvestDate,
            ...
            try
            {
                ...
                return Ok(new
                {
                    ...
                    data = new
                    {
                        ...
                        product.Price,
                        product.Stock,
                        ImageUrl = ValidateImageUrl(product.ImageUrl), // 验证图片文件是否存在
                        product.Category,
                        product.IsActive,
                        product.ActiveTime,
        ...
        public async Task<IActionResult> DeleteProduct(int id, [FromBody] DeleteProductRequest request)
        {
            try
            {
                ...
                // 检查权限：管理员可以删除任何产品，农户只能删除自己的产品
                var userRole = User.FindFirstValue(ClaimTypes.Role);
                if (userRole != "admin" && product.FarmerId != request.FarmerId)
                {
                ...
                {
                    orderCount = await _context.Orders.CountAsync(o => o.ProductId == id);
                }
                ...
                if (hasOrderReferences)
                {
                    ...
                    return Ok(new { 
                        code = 200, 
                        message = $"⚠️ 由于数据库外键约束限制，该产品无法永久删除。\n\n原因：该产品已被 {orderCount} 个订单引用，为保证订单数据完整性，系统已将产品设为下架状态。\n\n✅ 产品已成功下架，用户将无法继续购买此产品。",
                        isHardDeleted = false,
                        orderReferences = orderCount,
                        action = "downgraded" // 标识为降级操作（下架而非删除）
                    });
                }
        ...
        private string? ValidateImageUrl(string? imageUrl)
        {
            ...
            try
            {
                ...
                // 如果是完整的HTTP/HTTPS URL，直接返回（假设外部链接有效）
            ...
            {
                Console.WriteLine($"验证图片URL时出错: {ex.Message}");
                return null;
            }
        }
    }
