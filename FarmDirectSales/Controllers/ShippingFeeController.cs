    ...
    public class ShippingFeeController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetShippingFeeStatistics([FromQuery] DateTime? startDate, [FromQuery] DateTime? endDate)
        {
            try
            {
                ...
                // 从OrderGroups表获取运费统计数据（这是运费的真实来源）
                var orderGroups = await _context.OrderGroups
                    .Where(og => og.CreateTime >= startDate && og.CreateTime <= endDate)
                    .ToListAsync();

                // 获取对应的订单信息用于计算订单总数
                var orderGroupIds = orderGroups.Select(og => og.OrderGroupId).ToList();
                var orders = await _context.Orders
                    .Where(o => orderGroupIds.Contains(o.OrderGroupId.Value))
                    .Where(o => !o.IsDeleted) // 排除软删除的订单
                    .Where(o => o.Status != "已取消" && o.Status != "已退款") // 排除已取消和已退款的订单
                    .ToListAsync();

                // 计算统计数据（基于OrderGroups表的真实运费数据）
                decimal totalShippingFee = orderGroups.Sum(og => og.ShippingFeeAmount);
                decimal totalOrderAmount = orderGroups.Sum(og => og.TotalProductAmount); // 商品金额，不含运费
                int totalOrders = orders.Count; // 有效订单总数
                int freeShippingOrders = orderGroups.Count(og => og.ShippingFeeAmount == 0);
                
                // 计算每个运费规则的使用情况（基于OrderGroups）
                var ruleUsage = orderGroups
                    .Where(og => og.ShippingFeeId.HasValue)
                    .GroupBy(og => og.ShippingFeeId)
                    .Select(g => new
                    {
                        ShippingFeeId = g.Key,
                        OrderGroupCount = g.Count(),
                        TotalShippingFee = g.Sum(og => og.ShippingFeeAmount)
                    })
                    .ToList();
                ...
                var ruleStatistics = ruleUsage.Select(r => new
                {
                    ...
                    orderGroupCount = r.OrderGroupCount, // 订单组数量
                    totalShippingFee = r.TotalShippingFee,
                    averageFee = r.OrderGroupCount > 0 ? r.TotalShippingFee / r.OrderGroupCount : 0
                }).ToList();
                ...
                return Ok(new
                {
                    ...
                    data = new
                    {
                        ...
                        {
                            totalOrders, // 总订单数量
                            totalOrderGroups = orderGroups.Count, // 总订单组数量
                            totalShippingFee, // 总运费（来自OrderGroups）
                            totalOrderAmount, // 总商品金额（不含运费）
                            totalAmount = orderGroups.Sum(og => og.TotalAmount), // 总金额（含运费）
                            shippingFeePercentage = totalOrderAmount > 0 ? (totalShippingFee / totalOrderAmount) * 100 : 0,
                            freeShippingOrders, // 免运费的订单组数量
                            freeShippingPercentage = orderGroups.Count > 0 ? ((decimal)freeShippingOrders / orderGroups.Count) * 100 : 0,
                            averageShippingFee = orderGroups.Count > 0 ? totalShippingFee / orderGroups.Count : 0
                        },
                        ruleStatistics
                    }
                });
