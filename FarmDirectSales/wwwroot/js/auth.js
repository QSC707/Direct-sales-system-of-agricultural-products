    ...
    if (token && username && role) {
        ...
        if (!isDirectBuy) {
            ...
            if (currentPath && !currentPath.includes('login.html')) {
                // 检查当前页面是否为受保护的页面，避免将管理员页面设为普通用户的重定向目标
                const isAdminPage = currentPath.includes('/admin/');
                const isFarmerPage = currentPath.includes('/farmer/');
                
                // 只有当前页面不是受权限保护的页面时，才设置重定向
                if (!isAdminPage && !isFarmerPage) {
...
function updateNavbar(user) {
    if (typeof $ === 'undefined') {
        console.warn('jQuery未加载，使用原生JavaScript更新导航栏');
        updateNavbarNative(user);
        return;
    }
...
/**
 * 使用原生JavaScript更新导航栏状态（jQuery未加载时的备选方案）
 * @param {Object} user 用户信息
 */
function updateNavbarNative(user) {
    if (user) {
        // 已登录状态
        ...
        if (userDropdownEl) {
            userDropdownEl.style.display = 'block';
        }
        
        const loginItemEl = document.getElementById('loginItem');
        const registerItemEl = document.getElementById('registerItem');
        if (loginItemEl) loginItemEl.style.display = 'none';
        if (registerItemEl) registerItemEl.style.display = 'none';
        
        // 获取当前页面路径，判断是在哪个管理界面
        const currentPath = window.location.pathname;
        
        // 删除之前可能存在的特定角色链接
        const existingLinks = document.querySelectorAll('.admin-dashboard-link, .farmer-dashboard-link');
        ...
        if (dropdownMenu) {
            if (user.role === 'admin' && !currentPath.includes('/admin/')) {
                ...
                adminLink.className = 'dropdown-item admin-dashboard-link';
                adminLink.href = '/pages/admin/dashboard.html';
                adminLink.innerHTML = '<i class="fa fa-tachometer-alt mr-1"></i>管理中心';
                dropdownMenu.insertBefore(adminLink, dropdownMenu.firstChild);
            } else if (user.role === 'farmer' && !currentPath.includes('/farmer/')) {
                // 在普通页面，添加农户中心链接
                const farmerLink = document.createElement('a');
                farmerLink.className = 'dropdown-item farmer-dashboard-link';
                farmerLink.href = '/pages/farmer/dashboard.html';
                farmerLink.innerHTML = '<i class="fa fa-tachometer-alt mr-1"></i>农户中心';
                dropdownMenu.insertBefore(farmerLink, dropdownMenu.firstChild);
            }
        }
    } else {
        // 未登录状态
        ...
        const loginItemEl = document.getElementById('loginItem');
        const registerItemEl = document.getElementById('registerItem');
        if (loginItemEl) loginItemEl.style.display = 'block';
        if (registerItemEl) registerItemEl.style.display = 'block';
